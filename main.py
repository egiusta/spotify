...
def get_single_song(bot, update):
    ...
    logging.log(logging.INFO, f'mesajı sorgulamaya başla {message_id} in chat:{chat_id} from {username}')
    ...
    bot.send_message(chat_id=chat_id, text="getiriliyor...")
    ...
    if config["SPOTDL_DOWNLOADER"]:
        ...
        logging.log(logging.ERROR, 'indiricilerden birini seçmelisiniz')
...
def authenticate(bot, update):
    ...
    if update.effective_message.text == config["AUTH"]["PASSWORD"]:
        ...
        update_config()
        bot.send_message(chat_id=chat_id, text="Başarıyla giriş yaptınız. Keyfini çıkarın🍻")
        raise Exception("Signed In")
    elif chat_id not in config["AUTH"]["USERS"]:
        logging.log(logging.INFO, f'not authenticated try')
        bot.send_message(chat_id=chat_id, text="⚠️Bu bot kişiseldir ve oturum açmadınız. Lütfen girin"
                                               "oturum açmak için şifre. Bilmiyorsanız bot sahibiyle iletişime geçin.")
